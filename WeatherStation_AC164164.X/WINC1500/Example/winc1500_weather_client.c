...
/** Wi-Fi AP Settings. */
#define WLAN_SSID              "Redmi"             // target AP
#define WLAN_AUTH              M2M_WIFI_SEC_WPA_PSK   // AP Security 
#define WLAN_PSK               "21152115"            // security password
...
#define POST_BUFFER            "&appid=c592e14137c3471fa9627b44f6649db4&cnt=15&units=metric HTTP/1.1\r\nHost: api.openweathermap.org\r\nAccept: */*\r\n\r\n"
#define WEATHER_SERVER_NAME    "api.openweathermap.org"  // Weather information provider server
#define CITY_NAME              "Krakow"    // Input City Name
...
void ApplicationTask(void)
{
    ...
    case APP_STATE_START:
        printf("\r\n=========\r\n");            
        printf("Praca inz Gabriel Szklarek\r\n");
        printf("ssid: %s\r\n", WLAN_SSID);
        printf("host: %s\r\n", WEATHER_SERVER_NAME);
        printf("port: %u\r\n", SERVER_PORT);
        printf("=========\r\n");
        ...
        printf("=========\r\n");
        printf("-- Program reading weather forecast from the Internet --\r\n");
        printf("Starting ...\r\n");
...
struct WeatherData{
    float temperature; //temperature in C degree
...
int find_id(char* string) {
    char* descriptions[] = {"thunderstorm with light rain", "	thunderstorm with rain", "thunderstorm with heavy rain",
                        ...
                        "overcast clouds", "light intensity drizzle", "drizzle", "heavy intensity drizzle", "light intensity drizzle rain",
                        ...
                        "Shower sleet", "Light rain and snow", "Rain and snow", "Light shower snow", "Shower snow", "Heavy shower snow", "mist",
                        "Smoke", "Haze", "sand/ dust whirls", "fog", "sand", "dust", "volcanic ash", "squalls", "tornado", "clear sky", "few clouds", "scattered clouds", "broken clouds"};
    int codes[] = {200, 201, 202, 210, 211, 212, 221, 230, 231, 232, 
                   804, 300, 301, 302, 310, 311, 312, 313, 314, 321,
    ...
    for (int i = 0; i < sizeof(codes)/sizeof(int); i++) {
        if (strcmp(descriptions[i], string) == 0) {
            return codes[i];
        }
    }
    return 420;
...
static void socket_cb(SOCKET sock, uint8_t message, void *pvMsg)
{
    ...
    if (sock == tcp_client_socket) 
    {
        ...
        case M2M_SOCKET_RECV_EVENT:
        {
            ...
            {
                //get temp
                pcIndxPtr = strstr((char *)pstrRecv->p_rxBuf, "\"main\":{\"temp\":");
                if (NULL != pcIndxPtr) 
                {
                    pcIndxPtr = pcIndxPtr + strlen("\"main\":{\"temp\":");
                    pcEndPtr = strstr(pcIndxPtr, ",");
                    if (NULL != pcEndPtr) 
                    {
                        *pcEndPtr = 0;
                    }
                    data.temperature = atof(pcIndxPtr);
                    printf("Temperature: %s\r\n", pcIndxPtr);
                } 
                //get weather condition
                pcIndxPtrId = strstr(pcEndPtr + 1, "id\":");
                char* buffer_id;
                for (int x = 4; x < 7; x++ ) {
                    buffer_id += pcIndxPtrId[x];
                }
                pcIndxPtr1 = strstr(pcIndxPtrId + 1, "description\":");
                if (NULL != pcIndxPtr1) 
                {
                    pcIndxPtr1 = pcIndxPtr1 + strlen("description\":") + 1;
                    pcEndPtr1 = strstr(pcIndxPtr1, ",") - 1;
                    if (NULL != pcEndPtr1) 
                    {
                        *pcEndPtr1 = 0;
                    }
                    data.conditions = find_id(pcIndxPtr1);
                ...
                pcIndxPtr2 = strstr(pcEndPtr1 + 1, "dt_txt\":");
                if (NULL != pcIndxPtr1) 
                {
                    pcIndxPtr2 = pcIndxPtr2 + strlen("dt_txt\":") + 1;
                    printf("Weather forecast for: ");
                    for (int j = 0; j < 19; j++) {
                        printf("%c", pcIndxPtr2[j]);
                    }
                    printf("\r\n");
                } 
                else 
